...
def draw_summary_ui(total_cnt, success_cnt, fail_cnt):
    if total_cnt == 0: return
    
    # 실패율에 따라 박스 테두리 색상 변경
    border_color = "#FF5252" if fail_cnt > 0 else "#4FC3F7"
    emoji = "🚨" if fail_cnt > 0 else "🤖"
    
    st.markdown(f"""
    <div style="background-color: #262626; border: 1px solid {border_color}; border-radius: 12px; padding: 20px; margin-bottom: 25px; box-shadow: 0 4px 10px rgba(0,0,0,0.3);">
        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
            <h3 style="color: #E0E0E0; margin: 0; font-size: 18px;">{emoji} AI 분석 요약</h3>
            <span style="background: #333; padding: 4px 10px; border-radius: 15px; font-size: 12px; color: #aaa;">Log Analysis Agent</span>
        </div>
        <div style="font-size: 16px; color: #cccccc; line-height: 1.6;">
            요청하신 키워드로 분석을 완료했습니다.<br>
            총 <strong style="color: white; font-size: 18px;">{total_cnt}</strong>건의 거래 흐름이 감지되었습니다. 
            (<span style="color: #00C853; font-weight:bold;">정상 {success_cnt}</span> / 
             <span style="color: #FF5252; font-weight:bold;">에러 {fail_cnt}</span>)
        </div>
    </div>
    """, unsafe_allow_html=True)

def search_simple_text_upload(files, keyword):
    html_parts = []
    excel_data_list = []
    found_any = False
    ...
    return found_any, "".join(html_parts), excel_data_list, total_found_lines

def analyze_flow_web_upload(files, target_keyword, flow_list, mode, validator_step, category_name):
    html_parts = []
    ...
    if not flow_list: return False, "", [], 0, 0, 0
    ...
    for uploaded_file in files:
        ...
        for keyword_line_index in keyword_indices:
            ...
            for i in range(keyword_line_index, -1, -1): 
                if start_step_marker in "".join(file_lines[i].split()).lower(): start_idx = i; break
            end_idx = len(file_lines)
            for i in range(keyword_line_index + 1, len(file_lines)):
                if start_step_marker in "".join(file_lines[i].split()).lower(): end_idx = i; break
            
            is_duplicate = False
            for r_start, r_end in processed_ranges:
                if r_start == start_idx and r_end == end_idx: is_duplicate = True; break
            ...
            for line in target_lines:
                ...
                if rate_match:
                    collected_order_rates.append(rate_match.group(1))
            ...
            if not has_critical_error:
                ...
                if last_step_marker in full_block: is_last_step_found = True
            
            if has_critical_error: 
                status_html = "<span class='status-error'>🚨 에러</span>"
                stat_fail += 1
            elif is_last_step_found: 
                status_html = "<span class='status-normal'>✅ 정상</span>"
                stat_success += 1
            elif missing_steps: 
                status_html = "<span class='status-cancel'>⚠️ 취소</span>"
                stat_fail += 1
            else: 
                status_html = "<span class='status-normal'>✅ 정상</span>"
                stat_success += 1
                ...
                if step_found:
                    ...
                    for scan_idx in range(step_line_idx + 1, len(target_lines)):
                        ...
                        if (mode in ["CREDIT", "CHARGE"]) and "C_I_CREDIT" in step_name:
                            ...
                            if "결제 성공" in current_line: html_parts.append(f"<div class='step-pass'>&nbsp;&nbsp;&nbsp;&nbsp;✅ [결제 완료]</div>"); found_credit = True
            ...
            if pre_calc_cash or collected_order_rates:
                html_parts.append("<div class='money-box'>")
                
                if collected_order_rates:
                    html_parts.append("<div class='info-money'>💰 [투입 금액 상세 요약]</div>")
                    for rate in collected_order_rates:
                        html_parts.append(f"<div style='color:#81D4FA; font-size:13px; font-family:monospace;'>&nbsp;&nbsp;{rate}</div>")
                    if pre_calc_cash:
                        html_parts.append("<div style='margin-bottom:15px;'></div>") 
    ...
    return found_any_target, "".join(html_parts), excel_data_list, stat_total, stat_success, stat_fail
...
with st.sidebar:
    st.header("📂 파일 선택")
    uploaded_files = st.file_uploader("분석할 로그 파일을 드래그하거나 선택하세요", accept_multiple_files=True, type=['txt', 'log'])
    st.markdown("---")
        ...
        with st.spinner('업로드된 파일을 분석 중입니다...'):
            ...
            if search_mode == "거래 정밀 분석":
                ...
                for category_name, config in target_configs:
                    ...
                    found, html_res, excel_list, c_tot, c_suc, c_fail = analyze_flow_web_upload(uploaded_files, processed_keyword, flow_list, mode, validator, category_name)
                    if found:
                        found_total = True
                ...
                found_total, final_html, final_excel, grand_total = search_simple_text_upload(uploaded_files, keyword)
                grand_success = grand_total 
                grand_fail = 0

            if found_total:
                draw_summary_ui(grand_total, grand_success, grand_fail)
