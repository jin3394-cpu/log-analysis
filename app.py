    ...
    for uploaded_file in files:
        ...
        for keyword_line_index in keyword_indices:
            ...
            for line in target_lines:
                ...
                if "order_rate" in line.lower():
                    collected_order_rates.append(line.strip())
            ...
            # [수정] 돈 요약 박스 출력 (환율 정보 포함)
            if pre_calc_cash or collected_order_rates:
                html_parts.append("<div class='money-box'>")
                
                # 1. 환율 정보
                if collected_order_rates:
                    html_parts.append("<div class='info-money' style='margin-bottom:5px; border-bottom:1px dashed #777; padding-bottom:5px;'>💱 [적용 환율 정보]</div>")
                    ...
                    if pre_calc_cash:
                        html_parts.append("<div style='margin-bottom:15px;'></div>") # 간격 추가
        ...
        with st.spinner('업로드된 파일을 분석 중입니다...'):
            ...
            if search_mode == "거래 정밀 분석":
                ...
                for category_name, config in target_configs:
                    ...
                    found, html_res, excel_list = analyze_flow_web_upload(uploaded_files, processed_keyword, flow_list, mode, validator, start_date, end_date, category_name)
                    if found:
                        found_total = True
                ...
                found_total, final_html, final_excel = search_simple_text_upload(uploaded_files, keyword, start_date, end_date)
            if found_total:
